...
#' Parallel Bagging for FastKernelSurvivalSVM (with internal holdout)
#'
#' Fits an ensemble of models using bootstrap aggregation (bagging) and
#' computes predictions for \code{newdata}.
#'
...
#'   )
#'
#'   ## 4. Run Bagging with internal holdout for kernel weights
...
#'   cidx_test <- score(bag_results, test_df)
#'   cat(sprintf("Test C-index (Bagging): %.4f\n", cidx_test))
#' }
#' }
#'
...
fastsvm_bagging <- function(
  ...
  cli::cli_h1(paste(ji("rocket", ">>"), "FastKernelSurvivalSVM Bagging"))
  cli::cli_alert_info("Starting Bagging (B={B}, mtry={if (is.null(mtry)) 'All' else mtry}) on {cores} cores.")
  ...
  if (length(boot_results) == 0L) {
    cli::cli_alert_danger("Bagging failed completely: no valid bootstrap results.")
...
#' @param ... Not used.
#' @export
score.fastsvm_bag <- function(object, data, ...) {
...
print.fastsvm_bag <- function(x, ...) {
  ...
  cat("\n")
  cli::cli_h1(paste(ji("package", "#"), "FastKernelSurvivalSVM Bagging Result"))
  ...
  df_usage <- data.frame(
    ...
    Prob   = as.numeric(props), # Renomeado e mantido numérico (decimal)
    stringsAsFactors = FALSE
  )
  df_usage <- df_usage[order(-df_usage$Count), ]
  
  # Larguras Dinâmicas para Alinhamento Perfeito
  w_kern  <- max(nchar(df_usage$Kernel), nchar("Kernel"))
  w_count <- max(nchar(as.character(df_usage$Count)), nchar("Count"))
  w_prob  <- 6 # Espaço suficiente para "Prob" e "0.xxxx"
  
  # Cabeçalho da tabela com separadores verticais |
  cli::cli_rule()
  cat(sprintf(
    "%-*s | %*s | %s\n", 
    w_kern, cli::style_bold("Kernel"), w_count, cli::style_bold("Count"), cli::style_bold("Probability")
  ))
  cli::cli_rule()
  
  for (i in seq_len(nrow(df_usage))) {
    count_fmt <- sprintf("%*d", w_count, df_usage$Count[i])
    # Probabilidade em decimal (0.400) e não porcentagem
    prob_fmt  <- sprintf("%.4f", df_usage$Prob[i])
    
    # Destaque condicional se a frequência for alta (> 0.2)
    ...
    cat(sprintf(
      "%-*s | %s | %s\n",
      w_kern, df_usage$Kernel[i], count_fmt, prob_fmt
    ))
  }
  cli::cli_rule()
  ...
  # Ordenar: maiores probs primeiro (sobreviventes), depois eliminados
  df_w <- df_w[order(-df_w$Prob), ]
  
  w_kern_w <- max(nchar(df_w$Kernel), nchar("Kernel"))
  w_prob_w <- 11 # Para alinhar "Probability"
  
  cli::cli_rule()
  cat(sprintf(
    "%-*s | %*s | %s\n", 
    w_kern_w, cli::style_bold("Kernel"), w_prob_w, cli::style_bold("Probability"), cli::style_bold("Status")
  ))
  cli::cli_rule()
  ...
  for (i in seq_len(nrow(df_w))) {
    ...
    # Lógica de cores/status para o crop
    if (prob_val > 0) {
      # Texto "OK Selected" com check verde
      status_txt <- cli::col_green(paste0("OK Selected"))
      status_icon <- if(has_emo) ji("white_check_mark") else "OK"
      prob_fmt    <- cli::col_green(prob_fmt)
      
      # Formato final do status
      status_full <- paste(status_icon, cli::style_bold(cli::col_green("Selected")))
      
    } else {
      # Texto "X Eliminated" com x vermelho
      status_txt <- cli::col_red(paste0("X Eliminated"))
...
#' Predict method for fastsvm_bag
#'
